            ...
            <div className="mx-auto max-w-7xl px-6 lg:px-8">
                <div className="mx-auto max-w-2xl lg:mx-0">
                    ...
                    <p className="mt-8 text-lg font-medium text-gray-300 sm:text-xl/8">
                        Hệ thống đề xuất lộ trình du lịch thông minh dựa trên AI. TravelGO giúp bạn tiết kiệm thời gian và công sức trong việc lên kế hoạch cho chuyến đi.
                    </p>
                </div>
                <div className="mx-auto mt-10 max-w-2xl lg:mx-0 lg:max-w-none">
                    <dl className="mt-16 grid grid-cols-1 gap-8 sm:mt-20 sm:grid-cols-2 lg:grid-cols-5">
                        {members.map((member) => (
                            <div key={member.studentID} className="flex flex-col-reverse gap-1">
                                <dt className="text-base/7 text-gray-300">{member.name}</dt>
                                <dd className="text-4xl font-semibold text-white">{member.studentID}</dd>
                            </div>
                        ))}
                    </dl>
                </div>
