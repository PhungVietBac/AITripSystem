...
interface MapProps {
  searchQuery?: string;
  onSelectPlace?: (place: Place) => void; // Callback để chọn địa điểm
  selectedPlaceId?: string; // ID của địa điểm được chọn
  places?: Place[]; // Truyền danh sách places từ bên ngoài nếu cần
}
...
const MapComponent = ({ searchQuery, onSelectPlace, selectedPlaceId, places: externalPlaces }: MapProps) => {
  ...
  const [internalPlaces, setInternalPlaces] = useState<Place[]>([]);
  ...
  // Sử dụng places từ props nếu có, nếu không dùng từ API
  useEffect(() => {
    if (externalPlaces && externalPlaces.length > 0) {
      setInternalPlaces(externalPlaces);
      setMapCenter([externalPlaces[0].lat, externalPlaces[0].lon]);
      setMapZoom(6);
    } else if (initialPlaces && initialPlaces.length > 0) {
      setInternalPlaces(initialPlaces);
      setMapCenter([initialPlaces[0].lat, initialPlaces[0].lon]);
      setMapZoom(6);
    }
  }, [initialPlaces, externalPlaces]);
  ...
  const handleMarkerClick = (place: Place) => {
    ...
    setMapZoom(12);
    setShowPlaces(true);
    if (onSelectPlace) onSelectPlace(place); // Gọi callback khi chọn marker
  };
  ...
  // Cập nhật selectedPlace dựa trên selectedPlaceId từ props
  useEffect(() => {
    if (selectedPlaceId && internalPlaces.length > 0) {
      const place = internalPlaces.find((p) => p.idplace === selectedPlaceId);
      if (place) {
        setSelectedPlace(place);
        setMapCenter([place.lat, place.lon]);
        setMapZoom(12);
  ...
  }, [selectedPlaceId, internalPlaces]);

  // Lọc places dựa trên searchQuery
  const filteredPlaces = useMemo(() => {
    if (!searchQuery) return internalPlaces;
    return internalPlaces.filter((place) =>
      place.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      place.address.toLowerCase().includes(searchQuery.toLowerCase())
    );
  }, [searchQuery, internalPlaces]);
                  ...
                  {filteredPlaces &&
                    filteredPlaces.map((place) => (
                      <Marker
                        ...
                        icon={getPlaceIcon(place.idplace === (selectedPlace?.idplace || selectedPlaceId))}
                        eventHandlers={{
