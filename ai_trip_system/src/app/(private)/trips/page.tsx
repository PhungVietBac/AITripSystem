  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      const tripRequest: AIRequest = {
        ...
        days: getDays(formData.endDate, formData.startDate),
        time: formData.startDate,
        money: getBudgetLabel(formData.budget),
        transportation: getTransLabel(formData.transportation),
        travelStyle: getTravelStyleLabel(formData.travelStyle),
        interests: getInterestLabels(formData.interests),
        accommodation: getAccommodationLabel(formData.accommodation),
      };

      try {
        const token = await getCookie("token");
        console.log("Token:", token);
        const trip = await generateTrip(tripRequest, token ?? undefined);
        setData(trip);
        ...
        console.log(error);
        alert("Không tạo được kế hoạch du lịch");
      } finally {
