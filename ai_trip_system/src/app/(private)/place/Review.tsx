...
export default function Reviews({ idPlace }: ReviewProps) {
    ...
    const fetchReviews = async () => {
        ...
        try {
            ...
            const response = await fetch(
                `https://aitripsystem-api.onrender.com/api/v1/place_reviews/idPlace?lookup=${encodeURIComponent(idPlace)}&skip=0&limit=5`,
                {
                    headers: {
    ...
    const handleSubmitReview = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setSubmitError(err.message || 'Không thể gửi đánh giá. Vui lòng thử lại sau.');
            showToast(
                err.message || "Có lỗi xảy ra trong lúc cập nhật đánh giá!\n Vui lòng thử lại sau!",
                'error'
            );
        } finally {
            setSubmitting(false);
    ...
    return (
        <div className="mt-8">
            ...
            {showReviewForm && (
                <div className="bg-white rounded-lg shadow-md p-4 mb-6">
                    ...
                    <form onSubmit={handleSubmitReview}>
                        <div className='mb-4'>
                            <label htmlFor="name" className="block mb-2">Tên của bạn:</label>
                            <input
                                type="text"
                                id="name"
                                value={newName}
                        ...
                        <div className="flex space-x-3">
                            <button
                                type="submit"
                                disabled={submitting}
                                className={`px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition ${submitting ? 'opacity-70 cursor-not-allowed' : ''
                                    }`}
                            >
            ...
            {loading ? (
                ...
                <p className="text-gray-500 p-4 text-center">Chưa có đánh giá nào cho địa điểm này.</p>
            ) : (
                <div className="space-y-4">
                    {reviews.map((review) => (
                        <div key={review.idreview} className="bg-white rounded-lg shadow-md p-4">
                            <div className="flex items-center justify-between">
                                <p className="font-semibold">{review.name || 'Khách du lịch'}</p>
