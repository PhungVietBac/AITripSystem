...
export async function GET(request: NextRequest) {
    try {
        ...
        if (signature !== secureHash) {
            ...
            return NextResponse.redirect(new URL('/booking?payment=failed&error=signature', request.url));
        }
        ...
        if (responseCode === '00' && transactionStatus === '00') {
            ...
            return NextResponse.redirect(new URL('/booking?payment=success&vnp_ResponseCode=00', request.url));
        } else {
            // Payment failed
            console.log('VNPay payment failed:', responseCode, transactionStatus);
            return NextResponse.redirect(new URL(`/booking?payment=failed&vnp_ResponseCode=${responseCode}`, request.url));
        }

    } catch (error) {
        console.error('Error processing VNPay callback:', error);
        return NextResponse.redirect(new URL('/booking?payment=failed&error=processing', request.url));
    }
}
